...
seed = 0
directory = f'{data_name}_bd{batch_dim}_la{la}_do{dropout}_nc{n_critic}_me{metric}_ns{n_samples}_zd{z_dim}_epc{epochs}_se{save_every}_gu{repr(decoder_units)}_du{repr(discriminator_units)}_npseed{seed}'
